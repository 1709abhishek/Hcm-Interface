...
describe('HcmClient', () => {
  ...
  it('postDeduction applies and reports ok', async () => {
    const res = await client.postDeduction({
      idempotencyKey: 'k1',
      employeeId: 'e1',
      locationId: 'l1',
      amountDays: 3,
  ...
  it('getBatch returns the corpus', async () => {
    expect(await client.getBatch()).toEqual([
  ...
  it('throws HcmUnavailableError on timeout (chaos mode)', async () => {
    ...
    await expect(client.getBalance('e1', 'l1')).rejects.toThrow(
      HcmUnavailableError,
    );
  }, 10_000);
