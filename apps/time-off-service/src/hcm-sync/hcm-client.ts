...
export class HcmClient {
  constructor(
    private readonly baseUrl: string = process.env.HCM_BASE_URL ??
      'http://localhost:3001',
  ...
  async getBalance(
    ...
    const data = (await res.json()) as { balanceDays: number };
    return data.balanceDays;
  }
  ...
  async postDeduction(payload: DeductionPayload): Promise<DeductionResponse> {
    const res = await this.request('/deductions', {
      method: 'POST',
  ...
  async getBatch(): Promise<HcmBatchEntry[]> {
    ...
    if (!res.ok) throw new HcmUnavailableError(`HCM ${res.status}`);
    const data = (await res.json()) as { balances: HcmBatchEntry[] };
