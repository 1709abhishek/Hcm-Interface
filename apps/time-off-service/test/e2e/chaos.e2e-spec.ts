...
// Test file: res.body from supertest is intentionally untyped in e2e/chaos tests
...
describe('chaos e2e: service vs hostile HCM', () => {
  ...
  async function submitAndApprove(
    ...
    const res = await http()
      ...
      .expect(201);
    await http()
      .post(`/time-off-requests/${res.body.id}/approve`)
      .send({ managerId: 'm1' })
      .expect(200);
    return res.body.id;
  }
  ...
  it('HCM timeout: approval still instant; request recovers after HCM heals', async () => {
    ...
    await dispatcher.processOnce();
    expect((await http().get(`/time-off-requests/${id}`)).body.status).toBe(
      'APPROVED',
    );
  ...
  it('out-of-band clawback between approval and dispatch: SYNC_FAILED, hold released, drift visible', async () => {
    ...
    expect(
      drift.body.syncFailedRequests.map((r: { id: string }) => r.id),
    ).toContain(id);
  });
