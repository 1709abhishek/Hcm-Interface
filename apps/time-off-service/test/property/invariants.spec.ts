...
const opArb: fc.Arbitrary<Op> = fc.oneof(
  ...
  fc.record({
    kind: fc.constant<'batchSync'>('batchSync'),
    newBaseline: fc.integer({ min: 0, max: 30 }),
  }),
);
...
class Model {
  ...
  get available() {
    return this.baseline - this.taken - this.holds;
  }
  ...
  takeFirstPending(): { id: string; amount: number } | undefined {
    return this.pending.shift();
  }
  releaseHold(amount: number) {
    this.holds -= amount;
  }
  batchSync(newBaseline: number) {
        ...
        async (ops) => {
          ...
          try {
            ...
            await balances.applyBatch([
              { employeeId: EMP, locationId: LOC, balanceDays: 10 },
            ]);
            ...
            for (const op of ops) {
              ...
              const bBefore = await ds.manager.findOneByOrFail(Balance, {
                employeeId: EMP,
                locationId: LOC,
              });
              ...
              if (op.kind === 'submit') {
                ...
                try {
                  const req = await requests.submit(
                    { employeeId: EMP, locationId: LOC, amountDays: op.amount },
                    key,
                  );
              ...
              const b = await ds.manager.findOneByOrFail(Balance, {
                employeeId: EMP,
                locationId: LOC,
              });
              ...
              if (op.kind !== 'batchSync') {
                expect(available(b)).toBeGreaterThanOrEqual(
                  Math.min(0, availableBeforeOp),
                );
              }
